class zog_cl_load_data_btp definition
  public
  final
  create public .
...
class zog_cl_load_data_btp implementation.

  method if_oo_adt_classrun~main.

    data: lt_acc_categ type table of zog_t_acc_categ,
          lt_catego    type table of zog_t_categoria,
          lt_clientes  type table of zog_t_clientes,
          lt_clnts_lib type table of zog_t_clnts_lib,
          lt_libros    type table of zog_t_libros.
...
******** TABLA1 ********
    ...
    delete from zog_t_acc_categ.
    insert zog_t_acc_categ from table @lt_acc_categ.
...
******** TABLA2 ********
    ...
    "Delete possible entries; insert new entries
    delete from zog_t_categoria.
    insert zog_t_categoria from table @lt_catego.
    if sy-subrc eq 0.
...
******** TABLA3 ********
    ...
    "Delete possible entries; insert new entries
    delete from zog_t_clientes.
    insert zog_t_clientes from table @lt_clientes.
    if sy-subrc eq 0.
...
******** TABLA4
    ...
    "Delete possible entries; insert new entries
    delete from zog_t_clnts_lib.
    insert zog_t_clnts_lib from table @lt_clnts_lib.
    if sy-subrc eq 0.
      out->write( |Clientes Librería: { sy-dbcnt } registros insertados| ).
    endif.
...
******** TABLA5 ********
    ...
    'https://eltinteroeditorial.com/wp-content/uploads/2017/06/c%C3%B3mo-es-el-proceso-de-edici%C3%B3n.jpg' ) ).
    "Delete possible entries; insert new entries
    delete from zog_t_libros.
    insert zog_t_libros from table @lt_libros.
    if sy-subrc eq 0.
